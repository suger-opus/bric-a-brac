use crate::{
    ...
    models::{
        EdgeSchema, EdgeSchemaId, Graph, GraphId, GraphMetadata, GraphSchema, NewEdgeSchema,
        NewGraph, NewNodeSchema, NewPropertySchema, NodeSchema, NodeSchemaId, PropertyMetadata,
        PropertySchema, PropertySchemaId, PropertyType, Reddit, Role, UserId,
    },
};
...
RETURNING
    ...
    pub async fn create_properties(
        &self,
        connection: &mut PgConnection,
        new_properties: &[NewPropertySchema],
    ) -> Result<Vec<PropertySchema>, ApiError> {
        let mut ids = vec![];
...
impl TryFrom<Vec<SchemaRow>> for NodeSchema {
    ...
    fn try_from(schemas: Vec<SchemaRow>) -> Result<Self, Self::Error> {
        let first_row = if let Some(row) = schemas.first() {
            ...
            return Err(ApiError::ConversionDatabaseError(ApiErrorContent {
                message: "Node schema not found".to_string(),
                details: "No rows returned for node schema".to_string(),
            }));
        };

        let node_schema_id = first_row.node_schema_id.ok_or_else(|| {
            ApiError::ConversionDatabaseError(ApiErrorContent {
                message: "Missing node_schema_id".to_string(),
                details: "The node_schema_id is required for node schemas".to_string(),
            })
        })?;
...
impl TryFrom<Vec<SchemaRow>> for EdgeSchema {
    ...
    fn try_from(schemas: Vec<SchemaRow>) -> Result<Self, Self::Error> {
        let first_row = if let Some(row) = schemas.first() {
            ...
            return Err(ApiError::ConversionDatabaseError(ApiErrorContent {
                message: "Edge schema not found".to_string(),
                details: "No rows returned for edge schema".to_string(),
            }));
        };

        let edge_schema_id = first_row.edge_schema_id.ok_or_else(|| {
            ApiError::ConversionDatabaseError(ApiErrorContent {
                message: "Missing edge_schema_id".to_string(),
                details: "The edge_schema_id is required for edge schemas".to_string(),
            })
        })?;
