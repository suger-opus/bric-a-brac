use crate::dtos::graph_dto::PropertiesDto;
use crate::error::{ApiError, ApiErrorContent, ValidationContext};
use crate::models::{
    edge_schema_model::EdgeSchemaId,
    node_schema_model::NodeSchemaId,
    property_model::{Property, PropertyType},
};
...
impl ValidationService {
    ...
    pub async fn validate_node_data(
        ...
        if node_schema.node_schema.formatted_label != formatted_label {
            Err(ApiError::ValidationError(ApiErrorContent {
                message: format!(
                    "Formatted label '{}' does not match expected '{}'",
                    formatted_label, node_schema.node_schema.formatted_label
                ),
    ...
    pub async fn validate_edge_data(
        ...
        if edge_schema.edge_schema.formatted_label != formatted_label {
            Err(ApiError::ValidationError(ApiErrorContent {
                message: format!(
                    "Formatted label '{}' does not match expected '{}'",
                    formatted_label, edge_schema.edge_schema.formatted_label
                ),
                details: ValidationContext {
                    field: "formatted_label".to_string(),
                    issue: "Formatted label does not match schema".to_string(),
    ...
    fn validate_properties(
        &self,
        properties_data: &PropertiesDto,
        properties_schemas: &[Property],
    ) -> Result<(), ApiError> {
        properties_data
            .0
            .keys()
    ...
    fn validate_property(
        ...
        match (&property_schema.property_type, property_data_value) {
            (PropertyType::String, serde_json::Value::String(_)) => Ok(()),
            (PropertyType::Number, serde_json::Value::Number(_)) => Ok(()),
            (PropertyType::Boolean, serde_json::Value::Bool(_)) => Ok(()),
            (PropertyType::Select, serde_json::Value::String(value)) => {
