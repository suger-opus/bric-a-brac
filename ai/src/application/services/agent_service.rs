...
use crate::{
    application::{error::AgentError, AppError},
    infrastructure::{Message, MetadataClient, OpenRouterClient, ToolDefinition},
};
use bric_a_brac_dtos::{
    AgentEventDto, CreateSessionMessageDto, GraphSchemaDto, RoleDto, SessionDocumentIdDto,
    SessionIdDto, SessionMessageDto, SessionMessageRoleDto,
};
...
async fn run_agent_loop(
    ...
    let mut new_messages: Vec<CreateSessionMessageDto> = Vec::new();
    ...
    for (chunk_idx, chunk_content) in chunks.iter().enumerate() {
        ...
        loop {
            ...
            if result.tool_calls.is_empty() {
                ...
                new_messages.push(CreateSessionMessageDto {
...
fn build_tool_list(role: RoleDto) -> Vec<ToolDefinition> {
    ...
    match role {
        RoleDto::Owner | RoleDto::Admin | RoleDto::Editor => tools.extend(write_tools()),
        _ => {}
    }
